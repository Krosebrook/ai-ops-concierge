      ...
      <Card className="bg-white border-0 shadow-sm mb-6">
        <div className="p-4 space-y-3">
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <Select value={filterStatus} onValueChange={(v) => { setFilterStatus(v); resetSmartSort(); }}>
            ...
            <Select value={filterPriority} onValueChange={(v) => { setFilterPriority(v); resetSmartSort(); }}>
              <SelectTrigger className="w-40">
    ...
    <Dialog open={!!task} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl">
        ...
        <div className="space-y-6 py-4">
          ...
          {/* Automation source info */}
          {task.is_automated && (
            <div className="pt-4 border-t border-slate-100">
              <div className="flex items-start gap-2 p-3 bg-purple-50 border border-purple-100 rounded-lg">
                <Zap className="w-4 h-4 text-purple-600 mt-0.5 flex-shrink-0" />
                <div>
                  <p className="text-xs font-semibold text-purple-800">Auto-generated by Workflow Rule</p>
                  <p className="text-xs text-purple-600 mt-0.5">
                    Source: {task.source_entity_type || "Unknown"}{task.source_entity_id ? ` · ID: ${task.source_entity_id.slice(0, 8)}…` : ""}
                  </p>
                </div>
              </div>
            </div>
